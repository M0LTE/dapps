...
- **RHPv2 bearer: drive XRouter via RHP instead of AGW** *(open, deferred)*. The `scripts/sim-mixed-bearer.sh` work surfaced an architectural limitation in DAPPS-on-XR via AGW: XRouter scopes AGW callsign authorisation per-TCP-connection, so DAPPS's "open a fresh TCP connection per outbound forward" pattern fails (XR refuses the duplicate X-frame from the second connection because `AgwInboundService`'s connection still owns the call, and silently drops the subsequent C-frame). The cleanest fix is to skip AGW for XR entirely and use RHPv2 (Radio Host Protocol v2) as the second supported bearer alongside AGW. XRouter has shipped RHPv2 for years; mainline BPQ is expected to follow. The user has a vibe-coded RHPv2 client library at <https://github.com/M0LTE/rhp2lib-net> that DAPPS can take a dependency on; using it in DAPPS doubles as a hardening test for the library. Implementation shape: a new `Rhpv2SessionBackhaul : IDappsBackhaul` slotting under the existing seam (`Phase A0` already factored DAPPS to be bearer-agnostic via `IDappsBackhaul.SendAsync(BackhaulMessage, BackhaulRoute, ...)` and `IBackhaulInbox.DeliverAsync(...)`); a new `Rhpv2InboundService` BackgroundService that maintains a single RHPv2 connection and demuxes incoming sessions; per-neighbour bearer hint that picks RHPv2 over AGW when the host advertises both. Operator config: a new `DAPPS_RHP_HOST` / `DAPPS_RHP_PORT` env var pair; the `/Config` form gets matching fields; the dashboard's neighbour-add form gets a bearer selector. `AgwOutboundTransport` keeps its current shape for BPQ and gets the tolerance for `X` reply payload `0x00` (already in tree) so a future AGW-multiplexing fix can land cleanly without breaking that contract. The sim's XR-side daemons switch to RHPv2 once the bearer lands, unblocking the B->C and B->D paths.
