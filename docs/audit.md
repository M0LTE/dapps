...
Each row carries: timestamp, kind, bearer (`agw` / `udp` / `mqtt`), channel key (e.g. BPQ port byte), target callsign, message id (when forwarding a specific message), bytes, duration, success boolean, reason, and an error tag on failure.
